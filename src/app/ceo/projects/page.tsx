...
const ProjectsDashboard: React.FC = () => {
  ...
  const handleAddProject = async () => {
    ...
    try {
      const ownerEmail = localStorage.user_email;
      if (!ownerEmail) return alert("CEO email not found!");
      const body = { ...newProject, email: ownerEmail, members: newProject.team };
      const res = await fetch(
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      ...
      setSelectedProject(null);
      alert(`🗑 Project "${selectedProject.name}" deleted!`);
    } catch (err) {
      console.error(err);
  ...
  return (
    <DashboardLayout role="ceo">
      <div className="p-4 sm:p-6 bg-gray-50 min-h-screen">
        ...
        <div className="hidden md:block bg-white rounded-xl p-4 sm:p-6 shadow-sm border border-gray-100 overflow-x-auto">
          ...
          {isLoading ? (
            ...
            filteredProjects.map((project) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  {getStatusIcon(project.status)} <span>{project.status}</span>
                </div>
              </div>
            ))
          )}
